...
export const TokenCard = ({ token }: TokenCardProps) => {
  ...
  return (
    <Link 
      to={`/${id}`}
      className="grid-item flex flex-col items-center p-4 sm:p-6 bg-black/30 rounded-lg transition-all duration-300 hover:shadow-token-${id.toLowerCase()} font-orbitron group"
    >
      <div className="flex items-center mb-4 relative w-full">
        <img 
          ...
          alt={`${name} Logo`}
          className="token-logo w-[40px] h-[40px] sm:w-[50px] sm:h-[50px] rounded-full mr-3 transition-transform duration-300 group-hover:scale-110"
          loading="lazy"
          onError={(e) => {
        ...
        <div className="flex-1 min-w-0">
          <span className="text-lg sm:text-xl font-semibold text-[#cfd0d1] block truncate">
            {name} ({id.toUpperCase()})
          </span>
      ...
      <div className="w-full space-y-2 sm:space-y-3">
        ...
        <div className="flex justify-between text-sm sm:text-base">
          <span className="text-gray-400">$1000 Investment:</span>
